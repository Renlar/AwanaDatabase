...
import static com.liddev.awanadatabase.DatabaseWrapper.bookPrefix;
import static com.liddev.awanadatabase.DatabaseWrapper.completedPostfix;
import static com.liddev.awanadatabase.DatabaseWrapper.datePostfix;
...
public final class Book implements ItemListener {
    ...
    public boolean equals(Object o) {
        ...
        if (o.getClass().equals(this.getClass())) {
            Book os = (Book) o;
            if (!(os.sections.size() == this.sections.size())) {
                return false;
            }
            for (int i = 0; i < this.getNumberOfSections(); i++) {
                if (!this.getSection(i).equals(os.getSection(i))) {
                    return false;
                }
            }

            if (!(os.isCompleted() == this.isCompleted()
                    && os.getName().equals(this.getName())
                    && os.getGroup().equals(this.getGroup())
                    && os.getCompletionDate().equals(this.getCompletionDate()))) {
                return false;
            }
        }
        return true;
