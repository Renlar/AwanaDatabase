...
public final class Record {
    ...
    private static final ArrayList<Field> masterFieldList = new ArrayList<>();
    private static final ArrayList<Book> masterBookList = new ArrayList<>();
    private static final String[] fieldNameList = {
    ...
    private static final int[] fieldTypeDefaultDisplayLength = {
        10, 8, 5, 10, 15, 20, 30, 15, 20, 12, 10
    };
    private static final String fieldDefaultValue[] = {
    ...
    public boolean equals(Object o) {
        ...
        if (o.getClass().equals(this.getClass())) {
            Record s = (Record) o;
            if (fieldList.size() != s.fieldList.size() || books.size() != s.books.size()) {
                return false;
            }
            for (int i = 0; i < fieldList.size(); i++) {
                if(!(fieldList.get(i).equals(s.fieldList.get(i)))){
                    return false;
                }
            }
            for (int i = 0; i < books.size(); i++) {
                if(!(books.get(i).equals(s.books.get(i)))){
                    return false;
                }
            }
